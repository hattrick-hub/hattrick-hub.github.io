      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            // Deja de observar una vez animado (one-shot)
    ...
    if (totalItems > 1) {
      ...
      if (pauseBtn) {
        pauseBtn.addEventListener("click", () => {
          if (isPaused) {
            startAutoPlay();
            pauseBtn.querySelector("use")?.setAttribute("href", "#pause");
            pauseBtn.setAttribute("aria-label", "Pausar el carrusel");
            const visuallyHidden = pauseBtn.querySelector(".visually-hidden");
            if (visuallyHidden) visuallyHidden.textContent = "Pausar";
            isPaused = false;
          } else {
            stopAutoPlay();
            pauseBtn.querySelector("use")?.setAttribute("href", "#play");
            pauseBtn.setAttribute("aria-label", "Reanudar el carrusel");
            const visuallyHidden = pauseBtn.querySelector(".visually-hidden");
            if (visuallyHidden) visuallyHidden.textContent = "Reanudar";
            isPaused = true;
          }
        });
      }
